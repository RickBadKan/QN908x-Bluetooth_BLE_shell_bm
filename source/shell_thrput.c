...
#define mShellThrTxInterval_c              (0) /* ms */
...
static int8_t ShellThr_SetParams(uint8_t argc, char * argv[])
{
    ...
    while (argc > i)
    {
        if(!strcmp((char*)argv[i], "-c") && (mThrShellRole == thrPeripheral))
        {
            i++;
            gTroughputConfig.buffCnt = (uint16_t)atoi(argv[i]);
            i++;
        }
        else if(!strcmp((char*)argv[i], "-s") && (mThrShellRole == thrPeripheral))
        {
            i++;
        ...
        else if(!strcmp((char*)argv[i], "-ci") && (mThrShellRole == thrCentral))
        {
            i++;
...
static int8_t ShellThr_Start(uint8_t argc, char * argv[])
{
    ...
    {
        status = ShellThr_SetParams(argc - 1,  (char **)(&argv[1]));
    }
...
static void ShellThr_TxTimerCallback(void *p)
{
    App_PostCallbackMessage(ShellThr_SendData, p);
}
#endif
...
static void ShellThr_StartThroughputTest(thrGapRoles_t role)
{
    ...
    if (role == thrCentral) /* GAP Central */
    {
        ...
        if(gTmrInvalidTimerID_c != gThroughputTestTimerId)
        {
            uint32_t timeout = 3 * (mConnReqParams.connIntervalMax + mConnReqParams.connIntervalMax/4);
            TMR_StartIntervalTimer(gThroughputTestTimerId, timeout, ShellThr_CheckResults, NULL);
    ...
    else /* GAP Peripheral */
    {
        ...
        if (gTmrInvalidTimerID_c != gThroughputTestTimerId)
        {
            TMR_StartIntervalTimer(gThroughputTestTimerId, mShellThrTxInterval_c, ShellThr_TxTimerCallback, NULL);
            shell_write("Throughput test started.\n\rSending packets...\n\r");
        }
        else
        {
...
#else
        App_PostCallbackMessage(ShellThr_SendData, NULL);
        shell_write("Throughput test started.\n\rSending packets...\n\r");
#endif
    }
}
...
{
    (void)Gap_UpdateLeDataLength(gPeerDeviceId, gBleMaxTxOctets_c, gBleMaxTxTime_c);
}
/* EOF */