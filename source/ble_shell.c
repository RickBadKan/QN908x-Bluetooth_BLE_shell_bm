...
const char mpThrputHelp[]  = "\r\n"
           "thrput start tx [-c packet count] [-s payload size]\r\n"
           "thrput start rx [-ci min max]\r\n"
           "thrput stop\r\n";
...
void BleApp_GenericCallback (gapGenericEvent_t* pGenericEvent)
{
    ...
    if (pGenericEvent->eventType == gInitializationComplete_c)
    {
        ...
        //kannebley:autostart not optimized
        char gap[] = "gap";							//first array to simulate gap input
        char scanstart[] = "scanstart";				//second array to simulate scan command
        char* pAutoStart[2] = {gap, scanstart};		//pointer to both inputs
        mGapCmd.cmd(2, pAutoStart);					//simulate the keyboard input to start scanning
